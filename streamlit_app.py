...
def generaMag(df):
    fig = px.histogram(df, x="Magnitud", color_discrete_sequence=["red"], width=250, height=600)
    return fig

def generaProf(df):
    fig = px.histogram(df, x="Profundidad", color_discrete_sequence=["red"], width=250, height=600)
    return fig
...
# -----------------------------
col1, col2, col3 = st.columns([1.10,1.10,2.8])
with col1:
